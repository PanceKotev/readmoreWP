...
public class BookServiceImpl implements BookService {
    ...
    public List<BookReturn> listBooksByGenre(String genre) {
        return bookRepository.findAllByGenreEquals(genre).stream().map(book -> {
            int reviewSize=0;
            int likedBy=0;
            String seriesName="";
    ...
    public List<BookReturn> listBooksBySeries(String seriesName) {
        ...
        return bookRepository.findAllBySeries(series).stream().map(book -> {
            int reviewSize=0;
            int likedBy=0;
            if(book.getReviews()!=null)
                reviewSize=book.getReviews().size();
    ...
    public List<BookReturn> listBookByAuthor(String authorName) {
        ...
        return bookRepository.findAllByAuthorEquals(author).stream().map(book ->{
            int reviewSize=0;
            int likedBy=0;
            String seriesName="";
            if(book.getReviews()!=null)
